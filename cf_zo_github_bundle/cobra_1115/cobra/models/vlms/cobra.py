...
class CobraVLM(VLM):
    def __init__(
        ...
        # Vision toggle: default False to start in text-only mode (skip vision + projector).
    ...
    def set_enable_vision(self, enable: bool) -> None:
        """Convenience switch for turning vision on/off at runtime."""
        self.enable_vision = bool(enable)
    ...
    def from_pretrained(
        ...
        # Allow missing keys (e.g., CF-ZO mamba_scale not present in older checkpoints)
        missing, unexpected = vlm.llm_backbone.load_state_dict(
            model_state_dict["llm_backbone"], strict=False
        )
        if missing:
            overwatch.warning(f"[from_pretrained] Missing keys when loading llm_backbone: {missing}", ctx_level=1)
        if unexpected:
            overwatch.warning(f"[from_pretrained] Unexpected keys when loading llm_backbone: {unexpected}", ctx_level=1)
