...
# ==== FORCE repo path & cobra alias (for cobra-henry layout) ====
import sys, types
from pathlib import Path

_THIS = Path(__file__).resolve()
# repo_root = /work/hsuan1007/cobra
_REPO_ROOT = _THIS.parents[1]
# cobra_henry = /work/hsuan1007/cobra/cobra-henry
_COBRA_HENRY = _REPO_ROOT / "cobra-henry"

# 兩個路徑都丟進 sys.path
for p in (_REPO_ROOT, _COBRA_HENRY):
    if p.exists():
        sp = str(p)
        if sp not in sys.path:
            sys.path.insert(0, sp)

if "cobra" not in sys.modules and _COBRA_HENRY.exists():
    cobra_alias = types.ModuleType("cobra")
    cobra_alias.__path__ = [str(_COBRA_HENRY)]
    sys.modules["cobra"] = cobra_alias
# ===============================================================
