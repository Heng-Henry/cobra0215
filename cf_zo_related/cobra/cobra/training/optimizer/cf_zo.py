...
class _ScaleManager:
    ...
    def clamp_(self, min_value: float = 1e-3, max_value: float = 10) -> None:
        # 原本 min_value: float = 1e-3, max_value: float = 1e3
...
def calibrate_mamba_scales(
    ...
    try:
        for step in range(params.steps):
            ...
            loss_plus = _evaluate_entropy_align_loss(model, batch, device)
            scale_manager.load_vector(vector - params.perturb_radius * perturb)
            loss_minus = _evaluate_entropy_align_loss(model, batch, device)
            ...
            if params.log_frequency and step % params.log_frequency == 0:
                message = "[CF-ZO] step %d loss+=%.6f loss-=%.6f" % (
                    step,
                    loss_plus.item(),
                    loss_minus.item(),
                )
               # logger.info(message)
                ...
                base_loss = _evaluate_entropy_align_loss(model, batch, device)
                print(
                    f\"[CF-ZO][entropy-align] step {step} base={base_loss.item():.6f} |scale|_mean={vector.abs().mean().item():.6f}\",
                    flush=True,
                )
    finally:
        scale_manager.load_vector(vector)
